...
from rag.factual_rag import retrieve_facts
...
def generate_answer(company: str, question: Question) -> QuestionAnswer:
    ...
    prompt = _build_prompt(question, fact_chunks, style_examples)
...
def _build_prompt(
    question: Question, fact_chunks: list[dict], style_examples: list[dict]
) -> str:
    style_section = "\n\n".join(
    ...
    ) or "(no factual context retrieved)"

    # Incluir descripcion en la pregunta si existe
...
FACTUAL CONTEXT (answer using only this):
{factual_section}
